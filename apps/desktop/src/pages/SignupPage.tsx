...
export default function SignupPage({ onSignupSuccess, onGoToLogin }: SignupPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await window.electronAPI.signup(name.trim(), email.trim(), password);

      if (result.success && result.user) {
        onSignupSuccess(result.user, result.accessToken);
      } else {
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 flex flex-col items-center">
          ...
          <CardDescription className="text-center">
            Sign up to start using AI File Cleanup
          </CardDescription>
        </CardHeader>
        <CardContent>
